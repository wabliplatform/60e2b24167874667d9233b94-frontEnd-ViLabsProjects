let apiWorkpackageApi = new TempApi.WorkpackageApi();import TempApi from '../src/index';let apiProjectApi = new TempApi.ProjectApi();let project = new TempApi.Project();document.getElementById('icaj').onclick = (event) => {
    ...
    {  location.href= '/' ;}};document.getElementById('ix1q7').onclick = (event) => {
    event.preventDefault();
    {  location.href= '/' ;}};document.getElementById('i2lyj').onclick = (event) => {
    event.preventDefault();
    {  location.href= '/' ;}};document.getElementById('i4jic').onclick = (event) => {
    event.preventDefault();
    {  location.href= '/' ;}};document.getElementById('ie6a5').onclick = (event) => {
    event.preventDefault();
    {  location.href= '/' ;}};document.getElementById('i35mv3').onclick = (event) => {
    event.preventDefault();
    {  location.href= '/' ;}};document.getElementById('isu5yw').onclick = (event) => {
    event.preventDefault();
    {  location.href= '/' ;}};
...
document.getElementById("iyjqci").onclick = event => {
  ...
  [...advanceSelect.querySelectorAll("[annotationname]")].forEach(
    ...
    event.preventDefault();
    document.addEventListener('alignpCreate', function(e) {
  const advanceSelect = document.getElementById('i7w1n');
  const selectedElement = advanceSelect.getAttribute('selected-element');
  if (!selectedElement) return;
  [...advanceSelect.querySelectorAll("[annotationname]")].forEach(
    optionElement => {
...
});};window.onload = () => {document.addEventListener('alignwGetAll', function(e) {
